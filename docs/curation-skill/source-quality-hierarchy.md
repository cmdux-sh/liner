...
A URL is only a lead. The source is the readable content. Do not keep or trim a candidate from its URL, title, search snippet, crawler metadata, or your memory of the source. Keep/trim decisions require source evidence.
...
Every kept or trimmed source must include these fields in `working/03-evaluation.yaml`:

- `fetch_status: readable | partial` — `readable` means the substantive article, transcript, PDF body, or local content was retrieved. `partial` means enough real source content was retrieved to judge, such as an abstract plus substantial excerpt.
- `content_quality: high | medium` — `high` means dense, distinctive, and directly useful; `medium` means useful but selective. If the content is shallow, generic, AI-generated, SEO filler, a homepage/gallery without rationale, or a low-density transcript, drop it.
- `evidence:` — at least two bullets naming source-specific claims, examples, sections, methods, transcript moments, or limitations actually seen in the content.

`metadata_only` and `unavailable` candidates must be dropped. They can remain in the evaluation file as evaluated candidates, but they must not enter `tape.yaml` as normal corpus sources.

---

## JTBD-fit labels

Every kept or trimmed source must also get a `jtbd_fit` label. This prevents
topic adjacency from passing as source quality.

- **direct** — the source teaches, demonstrates, or argues for the core action
  in the JTBD.
- **bridge** — the source helps execute the result after the core decision is
  made: implementation mechanics, tokens, layout, process, API references, or
  operational constraints.
- **background** — broad vocabulary, primer, historical context, or generic
  orientation.

Bridge sources are valuable, but they are not the same as direct fit. If the job
is "turn moodboards into HTML interface concepts," then a moodboard analysis
method, a studio case study with rationale, or a talk on translating visual
references can be direct. A CSS layout guide, token spec, or design critique
process is a bridge unless it specifically teaches the translation move.

For a `reference-translation` capability, use an even stricter direct-fit test:
the source must help the future agent interpret the input/reference domain or
demonstrate the translation from reference qualities into target-output
decisions. Target-medium mechanics, accessibility standards, design-token
formats, style-guide infrastructure, and implementation docs are usually bridge
sources. They are important, but they cannot carry the core action alone.

Background sources should rarely be marked required. A kept background source
needs a narrow note explaining why its vocabulary or context is worth carrying.

If a knowledge-map section contains only `bridge` or `background` sources, mark
that as weak coverage in the rationale or quality report and backfill a direct
source if the section is central to the JTBD.

Quality also checks source roles at the corpus level. `reference / principle /
prescription / example` is only the generic kind distribution; some projects
need more specific roles. A visual art-director corpus may need high-craft case
studies and worked image-to-interface translations. A medical SEO corpus may
need clinical authority, search-intent evidence, and Google guidance. If those
role-specific sources are missing, write an operating-fit audit and recommend a
focused improvement pass instead of calling the corpus ready with a limitation.

---

## Bounded fetch fallback

Phase 4 should read enough to make honest decisions, but it must not stall on
unavailable content. For each candidate, try one direct fetch/read. If that
fails, make at most one recovery attempt: alternate transcript source, search
result, mirror, metadata page, or equivalent.

After two failed retrieval attempts total, stop searching for that candidate and
evaluate from verified metadata, the Phase 2 reason, and any partial content you
have. Usually that means `dropped`. If you have enough partial content to judge,
set `fetch_status: partial` and write the evidence bullets. If you only have
metadata, set the decision to `dropped`; the rationale can say the candidate
looked promising but was metadata-only. Every candidate still gets a decision in
`working/03-evaluation.yaml`; no source may keep the phase open indefinitely.
...
For large candidate sets, write decisions incrementally under `working/evaluation-decisions/` as section-sized YAML fragments. Use names like `01-foundations.yaml` and keep each fragment small enough to inspect. Liner assembles those fragments into `working/03-evaluation.yaml` in the original longlist order, so the fragment files are the durable work surface and `03-evaluation.yaml` is the canonical assembled artifact.
...
candidates:
  - url: https://example.com/source
    ...
    evidence:
      - Specific claim or example found in the readable source content.
      - Specific method, constraint, result, or limitation found in the content.
    section: foundations
    rationale: One-sentence reason this decision was made.
    note: >
      Role: Why this source belongs in the corpus. Value: What it contributes
      to the job-to-be-done. Limitations: What the source cannot prove.
```
