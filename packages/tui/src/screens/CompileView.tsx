...
import { streamCompile, runSetupJs, runShare, type CompileHandle } from "../ipc.js";
...
export function CompileView({
  ...
  // Share result — a persistent note below the boxes (not a transient
  // notification), so the curator can read the archive path at their leisure.
  ...
  function share(): void {
    void runShare(folder)
      .then((archivePath) => {
        setShareResult(`Shared → ${archivePath}`);
      })
      .catch((e: Error) => {
        setShareResult(`Share failed: ${e.message}`);
      });
  }
  ...
  return (
    <Box flexDirection="column">
              ...
              <Box flexDirection="column" marginTop={1}>
                ...
                {result.warnings.slice(0, 4).map((w, i) => (
                  <Text key={i} color={MUTED}>
                    {"  "}{w.severity === "error" ? "✗" : "⚠"} {truncate(w.url, 50)} — {truncate(w.message, 70)}
                  </Text>
                ))}
              </Box>
            ) : null}
      ...
      {/* Transient notification (e.g. "copied MIXTAPE.md"). Short-lived
          confirmations stay here; the share result is a persistent note
          below. */}
      {app.notification ? (
        <Box marginBottom={1}>
      ...
      {/* Share result — a persistent pink note below the boxes. Unlike the
          transient notification, this stays put so the curator can read (and
          copy) the archive path. */}
      {shareResult ? (
        <Box marginBottom={1}>
          <Text color={ERROR}>{shareResult}</Text>
        </Box>
      ) : null}
