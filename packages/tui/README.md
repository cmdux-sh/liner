...
For release testing on a machine that may already have Liner installed, first
run the hard-reset smoke commands in [PUBLISH-CHECKLIST.md](PUBLISH-CHECKLIST.md).
That removes global `linersh`, npx execution caches, Liner config/cache, and
Playwright Chromium before the external-user `npx linersh@<version>` test.
...
- Public release tarballs are generated by
  `scripts/release-tarballs.sh --version <version>` from the repo root. The
  script writes the local tarball folder, checksums, and the exact npm publish
  command block Arturo should run.
