...
> **Status:** This specification describes the **v1** format (with v1.1 additions for `local_file` sources and `render: js`) as implemented in the CLI. Additional source types planned for v2 are documented at the end.

---

## Overview

A Liner mixtape is a **folder**, not a single file. The folder contains the recipe, fetched source content, a synthesis, and a master file. Together they form a self-contained artifact ready to share, publish, or paste into an AI conversation.
...
Every Liner mixtape is a directory. The directory name is a slug — lowercase, hyphenated, filesystem-safe:
...
The folder is the canonical unit. Any operation that reads or writes a mixtape works on a folder, not on individual files.

A minimal valid project folder contains `tape.yaml`, `synthesis.md`, `MIXTAPE.md`, and a `sources/` directory with at least one file. The `working/` folder is recommended but not required. The `personal/` folder exists only when the recipe references at least one `local_file` source.

---

## `tape.yaml` — the recipe

The recipe lists the kept sources with curator notes, organized by section. It is the input to `liner compile`, which produces `MIXTAPE.md` and populates `sources/`.
...
| `sources` | list | At least one source entry |
...
v1 supports three source types: `youtube`, `web`, and `local_file`.

**Required fields per source:**

| Field | Type | Description |
|---|---|---|
| `type` | string | One of `youtube`, `web`, `local_file` |
| `url` | URL | Required for `youtube` and `web` source types. Omitted for `local_file` |
| `path` | string | Required for `local_file`. Relative path under `personal/` (e.g., `personal/foo.pdf`) |
...
`local_file` sources can reference content the curator has on disk — book chapters, gated articles, exported PDFs, Reader-Mode-saved HTML. The `path` must resolve under the project's `personal/` subdirectory; absolute paths and `..` traversal are rejected at validation. Supported extensions are `.md`, `.txt`, `.html`, `.htm`, `.pdf`. The maximum file size is 10MB.
...
sources:
  ...
  - type: web
    ...
    render: js
    note: Requires headless-browser rendering; install `linersh[js]` first.
    section: foundations
...
5. `sources` is a non-empty list
6. Every source has a `type` in the allowed set: `youtube`, `web`, `local_file`
...
10. For `local_file` sources: `path` is a relative POSIX path starting with `personal/` and contains no `..` segments
11. For `local_file` sources: the extension is one of `.md`, `.txt`, `.html`, `.htm`, `.pdf`
12. `youtube` and `web` sources must not declare `path` or `citation`; `local_file` sources must not declare `url`
...
In quick mode, the synthesis is AI-drafted during compilation. In methodology mode, the curator writes or substantially edits it. Either way, it lives at `synthesis.md` in the project folder and gets included verbatim at the top of `MIXTAPE.md`.
...
The evaluation file (`03-evaluation.yaml`) is structured — it lists every candidate source with the decision (kept, dropped, deferred) and the rationale. This is distinct from the cache (which holds source *content*); the evaluation file holds *decisions about* sources.
...
Source content fetched during research is stored in the curator's local cache, not in the project folder. The cache lives at `~/.liner/cache/` (or wherever `platformdirs` puts it on the user's OS).
...
When `liner compile` runs, it copies the cached content for kept sources into `sources/` in the project folder. The cache itself is not part of the mixtape and is not included in exports.
...
The archive contains the project folder at its root: `MIXTAPE.md`, `tape.yaml`, `synthesis.md`, `sources/`, `working/`, and `personal/` (when present). The archive does not contain anything from the shared content cache.

On import, the receiving Liner unzips the folder, then refetches any sources whose URLs aren't already in the receiving curator's cache. Sources already cached are reused. `local_file` sources are read from the archived `personal/` directly — no refetch.

Flags affecting the export:

- `--no-working-notes`: exclude `working/` from the archive
- `--no-personal`: exclude `personal/` from the archive. Used when the curator considers the local copies private (gated content, licensed material) or is preparing a library submission.
...
- **v1.1 additions** (additive, non-breaking): the `local_file` source type, the `render` field on `web` sources, and the `personal/` subdirectory. v1 parsers that don't know about these will reject tapes that use them — they're not silently forward-compatible. v1.1-aware parsers accept everything older v1 parsers accept.
...
- `content_file` (for `gated_url`) — path under `personal/` to a local file containing the saved content.
- `path` (for `local_media`) — path under `personal/` to the media file.
