...
export function LocalFilePicker({
  ...
  const localSourcesDir = join(projectFolder, "local-sources");
  // Lazily ensure local-sources/ exists.
  if (!existsSync(localSourcesDir)) {
    mkdirSync(localSourcesDir, { recursive: true });
  }

  const entries = useMemo<FileEntry[]>(() => listEntries(localSourcesDir, projectFolder), [
    localSourcesDir,
    projectFolder,
  ]);
  ...
  const [pasteValue, setPasteValue] = useState(initialPath ?? "local-sources/");
  ...
  if (pasteMode) {
    return (
      <Box flexDirection="column">
        ...
        <Text color={MUTED}>{localSourcesDir}/...</Text>
        <Box marginTop={1}>
          ...
          <TextInput
            ...
            placeholder="local-sources/foo.pdf"
          />
        </Box>
        <Box marginTop={1}>
          <KeyHints
  ...
  return (
    <Box flexDirection="column">
      <Text color={HEADING} bold>
        Select a file from local-sources/
      </Text>
      <Text color={MUTED}>{localSourcesDir}</Text>

      {entries.length === 0 ? (
        <Box marginTop={1} flexDirection="column">
          <Text color={MUTED}>(empty — put files into local-sources/ first, or press `n` to paste a path)</Text>
        </Box>
      ) : (
...
function listEntries(localSourcesDir: string, projectFolder: string): FileEntry[] {
  if (!existsSync(localSourcesDir)) return [];
  const out: FileEntry[] = [];
  for (const name of readdirSync(localSourcesDir)) {
    const abs = join(localSourcesDir, name);
    try {
