...
type SkillField = "path" | "url" | "note" | "section" | "priority";
type FieldKey = WebField | LocalField | SkillField;
...
const SKILL_FIELD_ORDER: SkillField[] = ["path", "url", "note", "section", "priority"];
...
export function SourceModal({
  ...
  function commit(): void {
    if (draft.type === "local_file") {
      ...
      if (!draft.path?.trim() && !draft.url?.trim()) {
        app.setNotification({ kind: "error", message: "skill needs a name/path or GitHub URL" });
        return;
      }
    } else {
      if (!draft.url?.trim()) {
  ...
  function toggleType(): void {
    const newType: SourceType =
      draft.type === "web" ? "local_file" : draft.type === "local_file" ? "skill" : "web";
  ...
  const headerSubtitle =
    draft.type === "local_file"
      ? "type: local_file"
      : draft.type === "skill"
        ? "type: skill source  ·  reads reference material only"
...
function placeholderFor(field: FieldKey, draft: SourceSpec): string {
  if (field === "url" && draft.type === "skill") {
    return "https://github.com/user/repo/tree/main/skills/name";
  }
  if (field === "url") {
  ...
  if (field === "path" && draft.type === "skill") {
    return "skill name, /path/to/skill, or local-sources/skills/name";
  }
  if (field === "path") return "local-sources/foo.pdf  (press enter to pick a file)";
